...
Use `148-engineering-checkpoint.json` with `148-frozen-files/`: **37 files**, comprising 22 owner files, 11 HTTP/UI files and four legacy-status files. Their saved originals and individual manifests remain in `148-originals/`, `148-owner-frozen-manifest.json`, `148-http-ui-frozen-manifest.json` and `148-legacy-frozen-manifest.json`. The owner's compiler_work.go before-hash is #147's frozen `6ee8b7e5ac6d51fac3bdf4e1eafba278664213e02e5af3a38bd6d4a74cec7367`, not the pre-#147 checkout. Its final bytes preserve #147 publication changes and add only #148 timing/reason hooks.
