...
record_path=root/'.scratch/memory-stage-4/142-checkpoint.json'
r=json.loads(record_path.read_text())
snapshot=pathlib.Path(tempfile.mkdtemp(prefix='evie-142-checkpoint-'))
...
p=root/'.scratch/memory-stage-4/verify-142-isolated.log'
r['snapshot']=str(snapshot);r['verification_log']=str(p);record_path.write_text(json.dumps(r,indent=2)+'\n')
print('Verifying isolated #142 tree '+r['tree']+' in '+str(snapshot),flush=True)
