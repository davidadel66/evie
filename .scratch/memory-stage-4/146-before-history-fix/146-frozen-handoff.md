...
- `ReviewDisclosure.tsx`: complete identity creation/reuse, global Predicate definitions, Aliases, Claims, sources/context, original contracted clock authority, correction lifecycle/validity, edit lineage and recursive compound member/record/dependency disclosure. A complete canonical expansion is available in addition to readable summaries.
...
- `go test ./internal/web -run 'CandidateReview|CandidateAdvancedHTTP' -count=1`: PASS, 1.859s.
- `go test -race ./internal/web -run 'CandidateReview|CandidateAdvancedHTTP' -count=1`: PASS, 38.306s.
- UI `npx tsc -b`: PASS.
- UI `npx vitest run`: PASS, 27 files / 146 tests, including the pre-existing user UI tests.
