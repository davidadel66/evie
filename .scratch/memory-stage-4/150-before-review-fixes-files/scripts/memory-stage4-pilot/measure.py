...
def validate_baseline_source(baseline, identity):
    ...
    for item in expected:
        name = item["path"]
        if (name.startswith("internal/") or name.startswith("cmd/")) and name.endswith(".go") or name in {"go.mod", "go.sum"}:
            if identity["files"].get(name) != item["sha256"]:
...
def summarize(raw):
    ...
    metrics["offered_foreground_events_per_second"] = raw["counts"]["foreground_source_events"] / foreground_elapsed
    ...
    for variant_id, variant in selected:
        for repetition in range(args.repetitions):
            ...
            for mode in modes:
                ...
                raw = json.loads(report_path.read_text()) if report_path.exists() and report_path.stat().st_size else {}
                entry = {"id": run_id, "variant": variant_id, "repetition": repetition+1, "mode": mode,
                         "command": command, "exit_code": process.returncode, "report_sha256": digest(report_path),
                         "resources_sha256": digest(resource_path), "sampled_peak_process_tree_rss_kib": resources["sampled_peak_process_tree_rss_kib"]}
                if process.returncode or raw.get("error") or not raw.get("disposable_database_removed"):
                    failures.append(run_id)
                    entry["status"] = "failed"
                    entry["error"] = raw.get("error", "worker did not produce a complete receipt")
                else:
                    entry["status"] = "passed"
                    entry["metrics"] = summarize(raw)
                    pair[mode] = entry
