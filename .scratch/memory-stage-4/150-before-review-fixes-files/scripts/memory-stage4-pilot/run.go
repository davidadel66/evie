...
func runWorkload(ctx context.Context, w workload) (r report, retErr error) {
	...
	r.Counts["foreground_source_events"] = int64(2 * w.Turns)
	...
	if w.Mode != "disabled" {
		...
		for {
			...
			if total >= want && unfinished == 0 {
				break
			}
			if time.Now().After(deadline) {
				return r, fmt.Errorf("catch-up deadline: jobs=%d expected=%d unfinished=%d", total, want, unfinished)
			}
			select {
			case <-ctx.Done():
				return r, ctx.Err()
