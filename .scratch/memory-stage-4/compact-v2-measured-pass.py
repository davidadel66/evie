...
base=pathlib.Path('cmd/evie/docs/fixtures/memory-stage-4-spike/v1/qwen-compact-v2');reports=base/'reports'
...
save('dispatch.json',{'at':now(),'authorization':'Parent dispatched after full verification and independent Standards/Spec passes','experiment_manifest_sha256':sha(base/'experiment-manifest.json'),'execution_plan_sha256':sha(base/'execution-plan.json'),'predispatch_sha256':sha(reports/'predispatch.json'),'binary_sha256':sha(pathlib.Path(command[command.index('--')+1])),'all_frozen_inputs_matched':True,'command':command,'owned_server_pid':pid})
