...
func selectCompilerUnitInTransaction(ctx context.Context, conn *sql.Conn, owner memory.ScopeContext, selection memory.CompilationSelection, generationID string, manifest []byte, generation memory.CompilerGeneration, scheduling compilerScheduling) (selectionID, jobID string, err error) {
	...
	err = func() error {
		...
		// Live discovery can encounter foreign-root coordinates between two
		// already owned intervals of this root. A successful bounded capture
		...
		// Explicit selections and every other source failure keep their outcome.
		emptyRootGap := scheduling.Lane == "new" && scheduling.AwaitClosure && state == "failed" && reason == "empty_selection" && len(window.NewEventIDs) == 0
		if emptyRootGap {
