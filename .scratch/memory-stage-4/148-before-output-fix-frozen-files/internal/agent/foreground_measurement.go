...
// exactly once after Send and output handling. Storage failure is diagnostic;
// it must not turn an already committed assistant response into a failed turn.
func BeginResponseMeasurement(ctx context.Context) (context.Context, func() error) {
	f := &foregroundFinalizer{}
	return context.WithValue(ctx, foregroundMeasurementKey{}, f), func() error {
		f.mu.Lock()
		defer f.mu.Unlock()
		if f.finished {
			return nil
