...
func validateEvieTableReferences(query string) error {
	...
	for i := 0; i < len(query); {
		...
		case isSQLWordByte(query[i]):
			...
			if expectTable {
				if _, allowed := evieQueryTables[token]; !allowed && !strings.HasPrefix(token, "MEMORY_COMPILER_") && !strings.HasPrefix(token, "MEMORY_REVIEW_") && !strings.HasPrefix(token, "SEMANTIC_") {
