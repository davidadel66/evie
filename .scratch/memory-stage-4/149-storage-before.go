...
func TestStage4GenericStorageConformance(t *testing.T) {
	...
	// Query the real initialized schema only to enumerate the protected families;
	// every model-facing read below must reject before opening any user database.
	...
	for _, name := range tables {
		for _, query := range []string{fmt.Sprintf(`SELECT * FROM "%s"`, name), fmt.Sprintf(`SELECT (SELECT COUNT(*) FROM main."%s") FROM jobs`, name)} {
			args, _ := json.Marshal(map[string]string{"db": "evie", "query": query})
			message, isErr, err := tools.BuiltinToolset().ExecuteWithApprovalAuthorizedCompletion(f.ctx, openrouter.ToolCall{ID: "conformance-query", Type: "function", Function: openrouter.FunctionCall{Name: "query_db", Arguments: string(args)}}, nil, nil, nil, nil)
			if err != nil || !isErr || bytes.Contains([]byte(message.Content), []byte("café")) {
				t.Fatalf("generic storage fence %s %t %v %s", name, isErr, err, message.Content)
			}
			denied++
		}
	}
	...
	stage4Evidence(t, "generic_storage_containment", map[string]any{"protected_tables_and_views": len(tables), "denied_model_queries": denied, "unaccepted_candidate_isolated": true, "privileged_shell_claim": "unchanged documented limitation"})
}
