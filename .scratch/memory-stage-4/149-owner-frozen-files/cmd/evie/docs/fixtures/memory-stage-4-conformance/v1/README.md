...
python3 scripts/memory-stage-4-conformance.py --fingerprint
python3 scripts/memory-stage-4-conformance.py \
  --browser-receipt /absolute/path/browser-receipt.json \
...
| Generic storage containment | Actual model-facing `query_db` execution rejects compiler, review, and semantic tables/views, including qualified and nested SQL forms. Unaccepted candidates remain absent from accepted reads. The privileged local shell is outside this model-tool boundary. |
...
Promotion checks. Global accepted-record browsing can use a separate active
global observer. Closed-source session destinations use owner operation
inspection plus exact accepted Store/projection checks; do not reopen the
...
The only production change owned by #149 initializes the first physical SQLite
