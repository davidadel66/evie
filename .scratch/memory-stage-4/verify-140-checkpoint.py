...
record_path=root/'.scratch/memory-stage-4/140-checkpoint.json'
r=json.loads(record_path.read_text())
snapshot=pathlib.Path(tempfile.mkdtemp(prefix='evie-140-checkpoint-'))
...
p=root/'.scratch/memory-stage-4/verify-140-isolated.log'
r['snapshot']=str(snapshot);r['verification_log']=str(p);record_path.write_text(json.dumps(r,indent=2)+'\n')
print('Verifying isolated #140 tree '+r['tree']+' in '+str(snapshot),flush=True)
