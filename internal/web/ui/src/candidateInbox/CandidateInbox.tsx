...
import { applicabilityLabel, MemoryScopeSelect, useMemoryPresentation } from "../memory/presentation";
...
export function CandidateInbox() {
  ...
  useEffect(() => {
    if (state.scope || !state.scopes || state.recovery) return;
    const scopes = state.scopes.scopes;
    const initial = scopes.find((scope) => scope.scope_key === preferredScope) ?? scopes.find((scope) => scope.scope_key === "global");
    if (initial) void controller.selectScope(initial.scope_key);
  }, [controller, preferredScope, state.scope, state.scopes, state.recovery]);
...
export function CandidateInboxView({ state, onScope, onScopes, onLoad, onInspect, onPrepare, onResolve, onRecover, onReason, controller }: {
  ...
  return <section aria-label="Candidate inbox" className="text-body flex min-h-0 flex-1 flex-col overflow-auto p-5 text-sm">
    <div className="border-hair flex flex-wrap items-center gap-3 border-b pb-4">
      <MemoryScopeSelect label="Review scope" scopes={state.scopes?.scopes ?? []} value={state.scope} onChange={onScope} />
      <div className="flex-1" />
      <button type="button" className={button} disabled={!state.scope || state.busy} onClick={() => onLoad()}>Refresh</button>
      <details><summary className="text-muted-text cursor-pointer px-2 py-2 text-sm">Options</summary><div className="mt-2 flex flex-wrap gap-2">
    ...
    {!state.scope && <p className="text-muted-text py-8">Choose a workspace to review memories.</p>}
    {state.page && <div className="border-hair my-4 rounded border">
      {state.page.candidates.length === 0 ? <p className="text-muted-text p-4">Nothing to review.</p> : <ul className="divide-hair divide-y">
    ...
    {item && <article className="border-hair bg-card my-2 rounded border p-4" aria-label="Candidate detail">
      ...
      {!item.redacted && <p className="text-teal mt-2 text-sm">Suggested scope: {item.candidate.proposal.destination === "everywhere" ? "Everywhere" : item.candidate.proposal.destination === "session" ? "This conversation" : item.candidate.proposal.destination === "workspace" ? "Workspace" : applicabilityLabel(item.destination, names)}</p>}
      ...
      {item.redacted ? <p className="text-amber-ink mt-3">The source is no longer eligible to display. Acceptance is blocked; rejection remains available.</p> : <>
        <CandidateSources title="Source" sources={item.candidate.support ?? []} />
        <CandidateSources title="Context only" sources={item.candidate.context ?? []} />
      ...
      {!item.redacted && <><details className="mt-3 text-sm"><summary>Memory details</summary><ProposalMeaning proposal={item.candidate.proposal}/></details>{item.edit && <EditLineage edit={item.edit}/>}<AdvancedReviewForms key={item.ref.candidate_id} state={state} controller={controller}/></>}
      {item.candidate.review_state === "unresolved" && <div className="mt-4 flex gap-3">
        <button type="button" className={button} disabled={state.busy || item.redacted} onClick={() => onPrepare("accept")}>Review change</button>
        <button type="button" className={button} disabled={state.busy} onClick={() => onPrepare("reject")}>Review rejection</button>
      </div>}
    </article>}
    {controller && ((state.page?.candidates.length ?? 0) > 1 || state.batchPreview || state.batchResult || state.batchPending) && <details className="my-4 text-sm" open={!!state.batchPreview || !!state.batchPending || !!state.batchResult}><summary className="text-muted-text cursor-pointer">Review together</summary><BatchReview key={`${state.scope}:${state.disclosureEpoch ?? 0}`} state={state} controller={controller}/></details>}
    {state.preview && <article className="border-teal-hair bg-panel my-4 rounded border p-4" aria-label="Exact review preview">
      ...
      {state.preview.action === "accept" ? <ReviewEffects preview={state.preview} /> : <p className="mt-3">Reject memory at the displayed interpretation and review revisions. No accepted memory will change.</p>}
      ...
      <button type="button" className={`${button} border-teal-hair text-teal`} disabled={state.busy || (state.preview.action === "accept" && !supportedPreview(state.preview))} onClick={onResolve}>{state.pending ? "Retry the same decision" : state.preview.action === "accept" ? "Save memory" : "Reject memory"}</button>
    </article>}
    {state.result && <article aria-label="Recorded review decision" className="border-hair my-4 rounded border p-4">
      ...
      {state.operation && <><ReviewEffects preview={state.operation.preview} /></>}
...
function candidateMeaning(item: OwnerCandidate): string {
  ...
  if (item.candidate.proposal.identity) return `${prop.polarity === "denied" ? "Denied: " : ""}${item.candidate.proposal.identity.subject?.name ?? prop.subject_entity_id} · ${item.candidate.proposal.identity.predicate?.label ?? prop.predicate_id} · ${item.candidate.proposal.identity.object?.name ?? prop.object.literal?.value ?? prop.object.entity_id}`;
