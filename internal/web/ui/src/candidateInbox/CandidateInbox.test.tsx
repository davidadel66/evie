...
describe("candidate review disclosure", () => {
  it("shows exact source, authority, scope, generation, reviewed effect and intentional approval", () => {
    const html = render();
    for (const text of ["Global", "Review scope", "Next candidate page", "More scopes", "I prefer café.", "owner_statement", "Recorded.", "none", "closed-session", "generation-1", "event-1", "evidence-hash", "David", "drink", "affirmed", "Unknown", "Create claim", "Supporting Source Links", "source-2", "Save memory", "preview-hash", "effect-hash"]) expect(html).toContain(text);
    expect(html).toContain("Context only"); expect(html).not.toContain("textarea");
  });
  ...
  it("shows redacted evidence without exposing retained proposal text and permits only rejection", () => {
    ...
    expect(html).not.toContain("café"); expect(html).not.toContain("Recorded."); expect(html).toContain("Acceptance is blocked"); expect(html).toContain("Review rejection");
    expect(html).toMatch(/disabled=""[^>]*>Review change/);
  });
  ...
  it("never offers acceptance of an undisclosed identity effect", () => {
    ...
    expect(html).toContain("Additional identity review"); expect(html).toMatch(/disabled=""[^>]*>Save memory/);
  });
  ...
  it("keeps the accepted memory view as the default and mounts suggestions separately", () => {
    ...
    expect(html).toContain("Existing accepted graph"); expect(html).toContain("Review"); expect(html).not.toContain("Candidate inbox");
  });
  ...
  it("makes retry, indexing and empty states actionable", () => {
    ...
    expect(html).toContain("Continue loading older scopes"); expect(html).toContain("Nothing to review"); expect(html).toContain("Retry the same decision"); expect(html).toContain('role="alert"');
  });
});

it("keeps denied polarity in an identity candidate heading before expansion", () => {
  const denied = { ...candidate, candidate: { ...candidate.candidate, proposal: { ...candidate.candidate.proposal, proposition: { ...candidate.candidate.proposal.proposition, polarity: "denied" }, identity: { subject: null, object: null, predicate: null, uncertainty: "", confidence: null } } } };
  const html = render({ ...state, detail: denied, preview: undefined });
  expect(html).toMatch(/<h3[^>]*>Denied: owner-1 · drink-1 · café<\/h3>/);
});

it("shows the actual scope for a legacy conversation candidate", () => {
  const html = render({ ...state, detail: { ...candidate, destination: "session:closed-session" }, preview: undefined });
  expect(html).toContain("Suggested scope: This conversation");
});
