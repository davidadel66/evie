...
export function MemoryActivity({ activity, onOpen }: { activity: MemoryActivityData; onOpen?: MemoryOpener }) {
  ...
  const label = unavailable ? "Memory unavailable" : activity.status === "cancelled" ? "Memory search cancelled" : activity.status === "exhausted" ? "Memory budget exhausted" : "Accepted memory";
  const detail = activity.status === "empty" ? "No matches" : `${activity.acceptedCount} supplied`;
