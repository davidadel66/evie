...
func TestKernelToolsetLeavesTodoToItsPlugin(t *testing.T) {
	wantAbsent := map[string]bool{"todo_list": true, "todo_add": true}
	for _, schema := range KernelToolset().Schemas() {
		if wantAbsent[schema.Function.Name] {
			t.Fatalf("Kernel Toolset still exposes plugin-owned schema %q", schema.Function.Name)
		}
	...
	if want := []string{"todo_list", "todo_add"}; !reflect.DeepEqual(legacyNames, want) {
		t.Fatalf("legacy Todo schemas = %v, want frozen compatibility schemas %v", legacyNames, want)
	}
}
