...
- Start with: /tutor the file-tools build from cmd/moussa/docs/active/file-tools.spec.md — resolve its four open questions first, then David writes read_file/edit_file.
...
- web_search — needs a keyed API (pick at build time)
- write_file — only if file-tools session keeps creation separate
- budget analysis verdict — ask moussa "how am I tracking against budget?"; transcript decides if step 5 needs a dedicated tool
- review flow live-fire — categorize awaiting-review transactions in chat via edit_db, mint rules from decisions

## Ideas
- web frontend (`moussa serve`) — rich output door (images/diffs/diagrams); decided over desktop/TUI, see decisions.md
- subagents — moussa spawns scoped sub-conversations; design session when long tasks clog one context
...
- 2026-07-16 cmd/agent → cmd/moussa; finance_sync/rules/categorize tools; internal/tools package
