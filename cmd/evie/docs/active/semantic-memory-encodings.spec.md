...
lifecycle effects below. Replay dispatches by the recorded version and fails
closed before changing a shadow projection when the version is unknown,
malformed, or paired with an operation kind that version does not define.
...
### Lifecycle operation encoding v3

The v3 accepted-operation envelope is identical to v1 and v2 except that
`schema_version` is `3` and `proposal.kind` is exactly one of
`retire_memory`, `restore_memory`, `retract_source`, or `restore_source`. The
normalized effect retains the ordered v1 fields from `scopes` through
`transitions` and adds one final `lifecycle_changes` field. Predicate, Entity,
Alias, Claim, Source Link, and Graph Link creation arrays are empty. The
`transitions` array contains the complete approved append-only state effect in
canonical target/dependency order.

`effect.scopes` is the canonical sorted set of scopes containing those
transitions. `prior_revisions` is the complete sorted authorization vector and
also contains any read-only global or Context Scope needed to authorize a
current-session target. A Workspace or project lifecycle target may therefore
record dependent current-session transitions without claiming a write to the
read-only global scope. Lifecycle preparation selects either the immutable
session Context Scope or, when explicitly requested, that same session's
`session:<id>` scope; it cannot name another write target. An active dependency
in a sibling session or unrelated Context Scope makes preparation fail without
enumerating private objects or changing semantic state; data dependencies never
derive additional scope authority.

`lifecycle_changes` contains exactly one record in this order:

1. `action`, exactly `retire`, `restore`, `retract_source`, or
   `restore_source`;
2. `object_kind`, exactly `entity`, `alias`, `claim`, or `source_link`;
3. the target `object_id`; and
4. `expected_state`, exactly the latest state from which the transition is
   legal: `active`, `retired`, `eligible`, or `retracted`.

An Entity retirement records the target Entity `retired` transition first,
then every active dependent Alias ordered by Alias ID, then every active
dependent Claim ordered by Claim ID. Its restoration records the exact inverse
set from that latest retirement operation in the same order and is invalid if
any member no longer has that retirement as its latest state. Alias and Claim
retirement or restoration contain only their target transition. Source Link
retraction and restoration contain only the same stable Source Link ID and
append `retracted` or `eligible`; the immutable Claim, locator, evidence hash,
creating operation, and evidence event are not recreated or rewritten.

The v3 proposal and effect hashes cover the complete transition array, target,
action, expected state, exact owner-request evidence event, and prior revision
vector. Applying a stale vector, incomplete dependency list, illegal latest
state, superseded Claim restoration, or incompatible Entity dependency writes
no operation, transition, or Scope Revision. A successful v3 operation
increments each scope containing one or more recorded transitions exactly once.

An existing database is upgraded by widening only the accepted-operation
version constraint from `1` or `1,2` to `1,2,3`. Every v1 and v2 row is copied
byte-for-byte and retains its original dispatcher, IDs, hashes, JSON, results,
...
The narrow v3 lifecycle contract is
[`manifest.schema.json`](../fixtures/semantic-memory/v3/manifest.schema.json).
It composes the frozen v1 operation and v2 correction definitions without
modifying them and adds the four v3 lifecycle kinds only. Its companion
[`lifecycle.json`](../fixtures/semantic-memory/v3/lifecycle.json) fixes one
compound Entity retire/restore cycle and one Source Link retract/restore cycle,
including complete transitions, expected prior states, proposal and effect
