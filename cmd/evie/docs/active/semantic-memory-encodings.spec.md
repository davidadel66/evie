...
Operation schema version `1` is the only version defined here. It is a positive
JSON integer and is stored beside every accepted operation. Replay dispatches by
the recorded version and fails closed before changing a shadow projection when
the version is unknown or malformed.
...
Changing any encoding or equality rule above requires a new operation and
fixture schema version plus a migration/replay decision; a DDL-only change may
not reinterpret accepted v1 history.
