...
class _Catalog:
    def __init__(self, seed):
        ...
        gold = (seed.get("case") or {}).get("gold")
        target_records = {record for support in (gold or {}).get("support_sets") or [] for record in support}
        # Only explicitly declared context is exempt from the question's gold
        # target. Required support wins if inconsistent labels name both.
        self.context_records = set((gold or {}).get("acceptable_context_record_ids") or []) - target_records
    ...
    def identities(self, item, binding):
        ...
        for kind, entity, alias_id in required - valid:
            if (kind == "entity" and not alias_id and entity == binding.get("subject_entity_id")
                    and claim.get("subject_entity_id") == entity):
                valid.add((kind, entity, alias_id))
                continue
            alias = self.aliases.get(alias_id)
            if (kind != "alias" or entity != binding.get("subject_entity_id")
                    or not alias or alias.get("entity_id") != entity):
                continue
            for source in item.get("sources") or []:
                original = self.links.get(source.get("source_link_id"))
                if (not original or alias.get("source_event_id") != source.get("event_id")
                        or alias.get("operation_id") != source.get("operation_id")
                        or alias.get("scope_key") != source.get("source_scope_key")):
                    continue
                self.source(original)
                value = alias.get("value")
                if (isinstance(value, str) and value
                        and re.search(r"(?<![\w-])" + re.escape(value) + r"(?![\w-])", source["evidence"])):
                    valid.add((kind, entity, alias_id))
        return required <= valid


def _historical_support(item, binding, question_target=True):
    expected = (binding.get("exact_reference") or {}) if question_target else {}
    # An eligible active episode establishes its original recorded wording in
    # either read mode. The oracle's query mode is not an additional fact.
    if (expected.get("intent") and item.get("intent") != expected["intent"]
            and (binding.get("claim_id") or binding.get("retired"))):
        return False
    ...
    if binding.get("claim_id"):
        ...
        if expected.get("valid_at_constrained"):
            # Read pins select a view; the canonical fact interval establishes
            # world-time support. Unknown bounds cannot prove a requested date.
            start, end = effective.get("from"), effective.get("to")
            if start is None or end is None:
                return False
            if not _instant(start) <= _instant(expected["valid_at"]) < _instant(end):
                return False
    elif expected.get("valid_at_constrained"):
        return False
    return True
        ...
        for index, item in enumerate(evidence or []):
            ...
            try:
                ...
                if kind == "accepted_memory":
                    ...
                    # A public immutable event does not restore a retracted
                    # canonical Source Link's accepted-evidence authority.
                    catalog.source(original)
                    ...
                    question_target = record not in catalog.context_records
                    if identity_supported and _historical_support(item, binding, question_target):
                    ...
                    for name, binding in catalog.bindings.items():
                        question_target = name not in catalog.context_records
                        if not binding.get("claim_id") and binding["source"]["event_id"] == event and _historical_support(item, binding, question_target):
