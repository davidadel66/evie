...
RETRIEVAL_CONFIGURATION = {
    ...
    "quotation_guidance": "verbatim-source-or-paraphrase-v1",
    ...
    "changed_conflict_scope_policy": "authorized-global-context-session-union-v1",
...
def verified_compiled_commit(repository, manifest_path):
    commit = subprocess.check_output(["git", "rev-parse", "--verify", "HEAD"],
                                     cwd=repository, text=True).strip()
    subject = subprocess.check_output(["git", "log", "-1", "--format=%s", commit],
                                      cwd=repository, text=True)
    if "#167" not in subject:
        raise RuntimeError("commit the verified #167 pilot before sealing the release inputs")
    sources = json.loads(Path(manifest_path).read_text())
    if not isinstance(sources, dict) or not sources:
        raise RuntimeError("compiled source manifest must contain frozen paths")
    for name, expected in sorted(sources.items()):
        committed = subprocess.run(["git", "show", commit + ":" + name], cwd=repository,
                                   stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if committed.returncode:
            raise RuntimeError("compiled source missing from verified commit: " + name)
        if hashlib.sha256(committed.stdout).hexdigest() != expected:
            raise RuntimeError("compiled source differs from verified commit: " + name)
    return commit
...
def seal_release(args):
    ...
    source_commit = verified_compiled_commit(repository, development_path.parent / "compiled-source-manifest.json")
    output = Path(args.output).resolve()
    output.mkdir(parents=True, exist_ok=False)
    shutil.copyfile(__file__, output / "run.py")
    ...
    frozen.update({
        ...
        "source_issue167_commit": source_commit,
        "workload_path": str(output / "workload.json"), "workload_sha256": sha256(output / "workload.json"),
        "inputs_path": str(inputs), "input_manifest_sha256": sha256(output / "input-manifest.json"),
        "prepared_inputs_archive_sha256": sha256(output / "prepared-inputs.tar.gz"),
