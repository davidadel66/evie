...
Original-event citation accuracy counts correct required and additional citations
over all required citations plus every additional citation. Missing required
citations stay in that denominator. The report retains required and additional
counts separately; a wrong additional citation cannot disappear from either the
accuracy calculation or the semantic-pass decision.
...
quotation or citation, failed behavior, reader error or empty answer. A personal-answer pass
...
The baseline exception permits missing citations only; it never permits a wrong
required or additional citation.
