...
Version 2 declares the sequence: mandatory canonical preparation, then index,
local, operating and real reader. Start index immediately after preparation and
sealing, on the declared machine without concurrent build or performance work.
Record bounded `/api/ps` responses before and after preparation and immediately
before index; actual observation, not execution order, identifies loaded state.
Preparation performs real required source indexing; there is no extra warmup
query, forced unload, retry or best-run selection. An absent loaded model is
reported as observed; every first-attempt outcome remains subject to the same
exact agreement and resource gates. Preserve all 24 index cases and exact
ordered source references, including `Paths`. Apply this same declared sequence
to the fresh held-out assessment. Record
...
A failed local turn has unknown final runtime work even when an earlier request
receipt exists. Retain that last receipt and the separately observed delegated
search duration, but do not substitute either for the final total. Any unknown
sample leaves the runtime-work quantile and gate incomplete across the full
20-sample denominator; known samples cannot establish a successful-only pass.

The retained v1 run exposed partial dense availability after an idle interval
longer than the selected model's 30-second keep-alive: accepted evidence retained
its exact source but acquired an additional `dense` path after rebuild, failing
the exact agreement gate. Supplementary single-case diagnostics are not cohort
replacements; the first may have overlapped a brief Go test, and the immediate
model-visible follow-up passed. Version 2 controls the comparison sequence and
records state while retaining this cold-start limitation. It does not claim
cold first-read dense reliability. Preparation cost includes any loading that
actually occurred; a cold load is never inferred solely from being first.
...
Release sealing independently rebuilds the development evidence report and every
assessment packet from retained raw reader results, then recomputes the full
resource/quality report. All compiled source bytes must match one verified #167
commit before release inputs are created. Report hashes alone do not establish
that an edited intermediate packet came from the actual reader.
...
## Source sufficiency, frozen before development v3
