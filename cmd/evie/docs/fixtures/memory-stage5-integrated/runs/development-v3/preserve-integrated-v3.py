#!/usr/bin/env python3
"""Plan, copy and verify immutable Stage 5 development-v3 evidence; stdlib only.
...
DIRECTORIES = {
    "raw-reader.tar.gz": ["integrated-development-v3-reader"],
    "raw-source-audit.tar.gz": ["integrated-development-v3-evidence"],
    "raw-assessments.tar.gz": ["integrated-development-v3-assessments"],
    "raw-local.tar.gz": ["integrated-development-v3-local"],
    "raw-index.tar.gz": ["integrated-development-v3-index"],
    "raw-operating.tar.gz": ["integrated-development-v3-operating"],
    "raw-verification.tar.gz": ["integrated-development-v3-checks"],
}
VERIFICATION_FILES = [
    "run-integrated-development-v3.py", "audit-development-v3.py", "grade-and-audit-development-v3.py",
    "167-verify-development-v3.py",
    "architecture-assessment-check-v3.json", "architecture-v3-assessment-summary.json", "architecture-v3-assessment-notes.md",
    "experiment-assessment-check-v3.json", "experiment-assessment-notes-v3.json",
    "integrated-development-v3-assessments-requirements-check.json",
    "integrated-development-v3-assessments-requirements-manifest.json",
    "integrated-development-v3-assessments-requirements-notes.md",
    "integrated-assessment-helper.py", "integrated-assessment-helper-notes.md",
    "author-architecture-v3-assessments.py", "v3-requirements-judgments.py",
]
REPO = Path("/Users/davidboktor/code/evie-memory-stage-5")
REPO_REGRESSION_DIR = Path("cmd/evie/docs/fixtures/memory-stage5-integrated/development/v3")

def repo_references():
    return {str(path.relative_to(REPO)): describe(path)
            for path in sorted((REPO / REPO_REGRESSION_DIR).rglob("*")) if path.is_file()}
...
def make_plan(root):
    ...
    frozen = root / "integrated-development-v3"
    freeze = read(frozen / "freeze.json")
    freeze_sha = digest(frozen / "freeze.json")
    if freeze["version"] != "integrated-development-v3" or freeze["partition"] != "development":
        raise ValueError("this tool preserves only the completed development-v3 run")
    ...
    for mode in ["reader", "local", "index", "operating"]:
        execution = read(root / ("integrated-development-v3-" + mode) / "execution.json")
    ...
    if not (root / "integrated-development-v3-resource/report.json").is_file():
        raise ValueError("resource audit report missing")
    evidence = read(root / "integrated-development-v3-evidence/evidence-report.json")
    if evidence["freeze_sha256"] != freeze_sha:
        raise ValueError("evidence report freeze identity mismatch")
    verify_map(root / "integrated-development-v3-reader", read(root / "integrated-development-v3-evidence/results-manifest.json"))
    assessments = read(root / "integrated-development-v3-assessment-manifest.json")
    if len(assessments) != 144:
        raise ValueError("144 assessment files required")
    verify_map(root / "integrated-development-v3-assessments", assessments)
    for path in (root / "integrated-development-v3-assessments").glob("*.json"):
        item = read(path)
    ...
    if len(evidence["cases"]) != 144 or sum(x["observed_model_calls"] for x in evidence["conditions"].values()) != 178:
        raise ValueError("actual reader totals differ from the reported 144 turns / 178 calls")
    for owner in ["architecture", "experiment", "requirements"]:
        files = list((root / ("integrated-development-v3-assessments-" + owner)).glob("*.json"))
    ...
    reports = {
        "reports/deterministic.json": "integrated-development-v3-deterministic.json",
        "reports/evidence-report.json": "integrated-development-v3-evidence/evidence-report.json",
        "reports/assessment-manifest.json": "integrated-development-v3-assessment-manifest.json",
        "reports/quality-report.json": "integrated-development-v3-quality/quality-report.json",
        "reports/resource-report.json": "integrated-development-v3-resource/report.json",
    }
    for name, original in reports.items():
        direct[name] = describe(root / original)
    for path in sorted(root.glob("integrated-development-v3*")):
        if path.is_file() and path.name not in set(reports.values()):
            direct["commands/" + path.name] = describe(path)
    for name in ["integrated-freeze-v3-command.json", "integrated-freeze-v3-console.log"]:
        direct["commands/" + name] = describe(root / name)
    direct["preserve-integrated-v3.py"] = describe(Path(__file__).resolve())
    for name, directories in DIRECTORIES.items():
        ...
        if name == "raw-verification.tar.gz":
            ...
            for path in sorted(root.glob("167-v3-*.log")):
                members[path.name] = describe(path)
            for path in sorted(root.glob("167-v3-*.json")):
                members[path.name] = describe(path)
        if name == "raw-assessments.tar.gz":
            members["integrated-development-v3-assessment-manifest.json"] = describe(root / "integrated-development-v3-assessment-manifest.json")
        counter = CountHash()
    ...
    return {"schema_version": 2, "source_root": str(root), "freeze_sha256": freeze_sha,
            ...
            "repository_reference_policy": "Existing v3 regression programs, exact red/green records, and decisions are hash-bound by repository-relative path without copying duplicates into this preserved run.",
            ...
            "excluded": [{"source": str(frozen / "integrated.test"), "bytes": (frozen / "integrated.test").stat().st_size,
                        ...
                        + [{"source": str(root / ("integrated-development-v3-assessments-" + owner)),
                            "reason": "identical final assessment bytes are in the combined 144-file assessment set"}
                           for owner in ["architecture", "experiment", "requirements"]],
            "completed_run": {"reader_turns": 144, "reader_http_calls": 178, "local_turns": 2880,
                "index_cases": 24, "operating_turns": 60, "semantic_assessments": 144,
                "quality_grader_exit_code": read(root / "integrated-development-v3-grade-execution.json")["exit_code"],
                "resource_grader_exit_code": read(root / "integrated-development-v3-resource-execution.json")["exit_code"],
                "release_ready": read(root / "integrated-development-v3-resource/report.json")["release_ready"]},
            "reproduction": {"original_paths_timestamps_and_contents_unchanged": True,
                ...
                "limits": ["This is development-v3 evidence, never held-out or release evidence.",
