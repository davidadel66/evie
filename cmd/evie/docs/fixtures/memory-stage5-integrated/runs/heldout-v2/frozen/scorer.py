...
# Pure report verification can load several freezes in one process. Resolve
# this freeze's sibling by path, never an unrelated cached audit_sources module.
_auditor_spec = importlib.util.spec_from_file_location(
    "integrated_bound_source_auditor", Path(__file__).with_name("audit_sources.py"))
_auditor = importlib.util.module_from_spec(_auditor_spec)
_auditor_spec.loader.exec_module(_auditor)
audit_evidence, merge_support = _auditor.audit_evidence, _auditor.merge_support
...
def build_report(freeze, results, freeze_sha256=None):
    """Reconstruct evidence and assessment inputs from raw results without writes."""
    ...
    report = {"freeze_sha256": freeze_sha256,
               ...
               "results_path": str(results), "results_manifest_sha256": digest(report_bytes(manifest)),
    ...
    return {"report": report, "results_manifest": manifest,
            "packets": {packet["case_id"] + "-" + packet["condition"] + ".json": packet for packet in packets}}


def evidence_report(freeze, results, output, freeze_sha256=None):
    generated = build_report(freeze, results, freeze_sha256)
    write(output / "results-manifest.json", generated["results_manifest"])
    write(output / "evidence-report.json", generated["report"])
    packet_directory = output / "assessment-packets"
    packet_directory.mkdir()
    for name, packet in generated["packets"].items():
        write(packet_directory / name, packet)
