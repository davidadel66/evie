...
class Report:
    ...
    def execution(self, directory, mode):
        ...
        self.executions[mode] = result
        if mode == "index":
            state = bound.get("selected_model_state_before_run")
            observed = bound.get("model_state_observed_at_utc")
            valid_observation = (isinstance(state, dict) and isinstance(state.get("models"), list)
                                 and all(isinstance(model, dict) for model in state["models"])
                                 and isinstance(observed, str))
            if valid_observation:
                valid_observation = datetime.fromisoformat(observed) <= datetime.fromisoformat(result["started_at_utc"])
            self.gate("index:actual_model_state_recorded", valid_observation, True, unit="boolean")
            self.summaries["index_initial_model_state"] = {"observed_at_utc": observed, "model_state": state,
                                                          "interpretation": "Recorded state only; absence is not a new availability gate."}
    ...
    def turn(self, dispatches, requests, calls, seed, case, final_accounting_observed=True):
    ...
    def local(self, directory):
        ...
        for case in self.cases:
            for condition in self.conditions:
                ...
                try:
                    ...
                    with gzip.open(path, "rt") as source:
                        for line in source:
                            ...
                            seen.add(repetition)
                            total += 1
                            dispatches, calls = row["dispatches"], row.get("kernel_calls") or []
                            final_receipt = ((dispatches[-1]["snapshot"].get("memory") or {}).get("investigation") if dispatches else None)
                            expected_total = None
                            basis = "failed_turn_final_work_unobserved"
                            if row.get("error") == "<nil>":
                                basis = "successful_turn_without_final_receipt"
                                if final_receipt is not None:
                                    expected_total = final_receipt.get("kernel_work_ns")
                                    basis = "successful_final_dispatch_receipt"
                                elif dispatches and not calls:
                                    expected_total = 0
                                    basis = "successful_turn_without_kernel_searches"
                            available = integer(expected_total)
                            accounting_matches = (type(row.get("runtime_accounting_available")) is bool
                                                  and row["runtime_accounting_available"] == available
                                                  and row.get("runtime_accounting_basis") == basis
                                                  and row.get("runtime_kernel_work_ns") == expected_total)
                            if not accounting_matches:
                                issues.append(str(repetition) + ": final runtime availability, basis or value differs from successful final dispatch")
                            known = available and accounting_matches
                            last_observed = next((((item["snapshot"].get("memory") or {}).get("investigation")) for item in reversed(dispatches)
                                                  if (item["snapshot"].get("memory") or {}).get("investigation") is not None), None)
                            if row.get("last_runtime_accounting") != last_observed:
                                issues.append(str(repetition) + ": last observed receipt differs; it cannot substitute for an unknown final total")
                            errors, metrics = self.turn(dispatches, row["encoded_requests"], calls, self.seeds[case["id"]], case,
                                                        final_accounting_observed=known)
                            ...
                            if row.get("kernel_searches") != metrics["kernel_searches"] or row.get("summed_observed_kernel_search_ns") != metrics["kernel_search_ns"]:
                            ...
                            values = [row.get("first_dispatch_elapsed_ns"), row.get("whole_turn_elapsed_ns"), row.get("summed_observed_kernel_search_ns")]
                            if not all(integer(value) for value in values) or values[0] <= 0 or values[1] < values[0]:
                                ...
                                for group, value in zip((first, whole, observed), values):
                                    group.append(value)
                            if known:
                                work.append(expected_total)
                ...
                runtime_complete = complete and len(work) == repetitions
                summary = {"case_id": case["id"], "condition": condition, "observed_repetitions": sorted(seen),
                           "complete": complete, "call_counters": counters, "call_counter_scope": "Kernel calls are observed through turn return; SearchAttempts belongs to the last observed receipt and may omit later failed-turn work", "kernel_status_counts": statuses, "first_dispatch": quantiles(first), "whole_turn": quantiles(whole),
                           "runtime_work": quantiles(work) if runtime_complete else None,
                           "runtime_accounting_complete": runtime_complete, "runtime_accounting_known_samples": len(work),
                           "runtime_accounting_unknown_samples": repetitions-len(work),
                           "observed_kernel_search": quantiles(observed), "observed_kernel_search_scope": "delegated SearchMemory duration only; lower bound when final runtime work is unknown", "violations": issues}
                ...
                self.gate("local:" + label + ":runtime_accounting_samples", len(work) if runtime_complete else None, repetitions)
                self.gate("local:" + label + ":work_p95", summary["runtime_work"]["p95_ns"] if runtime_complete else None, self.limits["local_retrieval_work_p95_ms_max"]*1000000, "<=", "nanoseconds")
    ...
    def quality(self, evidence_path, quality_path):
        ...
        self.reader_binding(evidence_path, evidence)
    ...
    def reader_binding(self, evidence_path, evidence):
        # Report hashes bind bytes, but do not prove that an intermediate report
        # or manually opened packet was derived from the original reader trace.
        generated = self.helper.build_report(self.freeze, Path(evidence["results_path"]), self.freeze_hash)
        self.gate("evidence:exact_raw_recomputation", generated["report"] == evidence, True, unit="boolean")
        directory = Path(evidence_path).parent / "assessment-packets"
        expected = generated["packets"]
        actual = {path.name for path in directory.glob("*.json")}
        self.gate("evidence:exact_raw_packet_identities", actual, set(expected), unit="set")
        mismatches = []
        for name in sorted(actual & set(expected)):
            if self.json(directory / name) != expected[name]:
                mismatches.append(name)
        self.gate("evidence:exact_raw_packets", len(mismatches), 0, details=mismatches)

    def quality_counts(self, quality):
        qrows = {(row["case_id"], row["condition"]): row for row in quality["cases"]}
        denominators, citations = [], []
        fields = ("required_citation_denominator", "additional_citation_denominator", "citation_denominator",
                  "required_citation_hits", "additional_citation_hits", "citation_hits")
        for case in self.cases:
            ...
            for condition in self.conditions:
                ...
                label = case["id"] + "/" + condition + ": "
                if row["component_denominator"] != components or row.get("required_citation_denominator") != len(supports) or row.get("assessment_valid") is not True:
                    denominators.append(case["id"] + "/" + condition + ": missing assessment or altered denominator")
                if not all(integer(row.get(key)) for key in fields):
                    citations.append(label + "citation counts must be nonnegative integers")
                    continue
                required, additional = row["required_citation_denominator"], row["additional_citation_denominator"]
                required_hits, additional_hits = row["required_citation_hits"], row["additional_citation_hits"]
                if (row["citation_denominator"] != required + additional or row["citation_hits"] != required_hits + additional_hits
                        or required_hits > required or additional_hits > additional):
                    citations.append(label + "required/additional citation arithmetic differs")
                if row.get("assessment_valid") is not True and row["citation_hits"] != 0:
                    citations.append(label + "invalid assessment cannot supply citation hits")
                if row.get("semantic_pass") is True and additional_hits != additional:
                    citations.append(label + "incorrect additional citation cannot pass semantically")
        self.gate("quality:required_denominators", len(denominators), 0, details=denominators)
        for condition in self.conditions:
            cohort = [qrows[(case["id"], condition)] for case in self.cases]
            if not all(integer(row.get(key)) for row in cohort for key in fields):
                continue
            summary = quality["conditions"][condition]
            for summary_key, row_key in (("required_correct_citations", "required_citation_hits"),
                                         ("required_citation_denominator", "required_citation_denominator"),
                                         ("additional_correct_citations", "additional_citation_hits"),
                                         ("additional_citation_denominator", "additional_citation_denominator"),
                                         ("correct_citations", "citation_hits"), ("citation_denominator", "citation_denominator")):
                if summary.get(summary_key) != sum(row[row_key] for row in cohort):
                    citations.append(condition + ": citation summary arithmetic differs for " + summary_key)
            total = sum(row["citation_denominator"] for row in cohort)
            accuracy = sum(row["citation_hits"] for row in cohort) / total if total else None
            if summary.get("original_event_citation_accuracy") != accuracy:
                citations.append(condition + ": original-event accuracy omits or miscounts required/additional citations")
        self.gate("quality:citation_arithmetic", len(citations), 0, details=citations)
    ...
    def sequence(self):
        order = ["index", "local", "operating", "reader"]
        self.gate("execution:declared_order", self.freeze.get("measurement_order"), order, unit="sequence")
        prepare = self.json(self.freeze_path.parent / "prepare-execution.json")
        self.gate("execution:mandatory_preparation_exit", prepare.get("exit_code"), 0)
        end = datetime.fromisoformat(prepare["finished_at_utc"])
        states = {}
        for point in ("before", "after"):
            state = self.json(self.freeze_path.parent / ("model-state-" + point + "-preparation.json"))
            observed = datetime.fromisoformat(state["observed_at_utc"])
            valid = isinstance(state.get("model_state"), dict) and isinstance(state["model_state"].get("models"), list)
            valid = valid and (observed <= datetime.fromisoformat(prepare["started_at_utc"]) if point == "before" else observed >= end)
            self.gate("execution:preparation_model_state:" + point, valid, True, unit="boolean")
            states[point] = state
        self.summaries["preparation_model_states"] = states
        rows = [{"stage": "preparation", "started_at_utc": prepare["started_at_utc"], "finished_at_utc": prepare["finished_at_utc"]}]
        for mode in order:
            entry = self.executions[mode]
            start, next_end = datetime.fromisoformat(entry["started_at_utc"]), datetime.fromisoformat(entry["finished_at_utc"])
            self.gate("execution:order:" + mode, end <= start <= next_end, True, unit="boolean")
            rows.append({"stage": mode, "started_at_utc": entry["started_at_utc"], "finished_at_utc": entry["finished_at_utc"]})
            end = next_end
        self.gate("execution:preparation_observation_before_index", datetime.fromisoformat(states["after"]["observed_at_utc"]) <= datetime.fromisoformat(self.executions["index"]["started_at_utc"]), True, unit="boolean")
        self.summaries["execution_order"] = rows
...
def build_report(freeze, evidence_report, quality_report, local, index, operating, deterministic):
    """Recompute from retained inputs without creating files or measurements."""
    paths = {key: str(Path(value).resolve()) for key, value in (
        ("freeze", freeze), ("evidence_report", evidence_report), ("quality_report", quality_report),
        ("local", local), ("index", index), ("operating", operating), ("deterministic", deterministic))}
    try:
        report = Report(paths["freeze"])
        report.stage("freeze", report.setup)
        if report.helper is not None:
            report.stage("quality", lambda: report.quality(paths["evidence_report"], paths["quality_report"]))
            report.stage("local", lambda: report.local(paths["local"]))
            report.stage("index", lambda: report.index(paths["index"]))
            report.stage("operating", lambda: report.operating(paths["operating"]))
            report.stage("deterministic", lambda: report.deterministic(paths["deterministic"]))
            report.stage("execution_order", report.sequence)
        ...
        result = {"schema_version": 1, "all_required_gates_pass": False, "release_ready": False, "complete": False,
                  ...
                  "failures": [str(error)]}
    result["measurement_inputs"] = paths
    # The API and CLI return identical JSON-native values, including set-valued
    # identity gates, so seal verification can compare the complete report.
...
def main():
    ...
    result = build_report(args.freeze, args.evidence_report, args.quality_report,
                          args.local, args.index, args.operating, args.deterministic)
