...
| --- | ---: | ---: | ---: |
| automatic-v2 | 215 | 590836 | 1008 |
| automatic-v3 | 214 | 587145 | 4323 |
...
For automatic-v2, three source files came from the existing retained
`../../v2/reproduce-source.patch`, applied to copies of the original #160 export.
Every resulting file matches its pre-existing freeze hash. The other files came
from exact matching original exports. No original bytes were inferred.

Run the lossless source/archive and original-freeze inventory verifier from the
repository root:

```sh
python3 -B cmd/evie/docs/fixtures/memory-stage5-automatic/source-preservation/v1/verify-source.py
```
