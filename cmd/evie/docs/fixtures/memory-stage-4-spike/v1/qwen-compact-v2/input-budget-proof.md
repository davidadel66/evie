# Closed-reference compact Qwen configuration: predispatch input proof

This is the separate `compact-v2` generator configuration under ticket #135.
It replaces only the output reference schema and its embedded prompt text.
The `compact-v1` projection, alias seal and strict canonical expansion are
unchanged; all original90 comparison requests remain immutable. The
...
checks those identities plus the independent compact-v2 prompt/schema constants;
changing either compact configuration does not authorize the other.
...
`6153 + compact_input_utf8_bytes + 80 + 2 + 768 + 64 <= 8192`.

Thus the input JSON can occupy at most 1125 bytes. The ten approved development
windows consume 282–696 bytes; the largest is N08-b. Its complete conservative
bound is 7763 tokens including the full output cap and reserve, leaving 429
...
The generator schema uses three closed `anyOf` alternatives for every supporting
and context reference: `ref` only; required `ref` plus `selector` whole/date;
or required `ref`, `selector` range, `start` and `end`. Minimum start/end are
0/1. Cross-field ordering, source bounds, UTF-8 scalar boundaries, clock
eligibility and source/context authority are still checked by the unchanged
adapter; schema membership alone does not prove them. The reference schema
does not claim to enforce alias membership or entailment.

The [offline grammar proof](grammar-proof/REPORT.md) records actual compilation
and consumption of the generated grammar from pinned Ollama0.6.3 source.
The runtime constrains property order to schema order. JSON-equivalent property
permutations remain accepted by the adapter but are not all generated by this
grammar; required-array order does not change that behavior.
